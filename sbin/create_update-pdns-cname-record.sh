...
USAGE="\
create_update-pdns-cname-record.sh [options] alias-name.domain.tld.
        target-name.target-domain.tld.

Creates a DNS CNAME record with the specified name and target.
...
if ! is_valid_forward_dns_name "$CNAME"; then
    >&2 echo "Record name '$CNAME' is not a correctly"
    >&2 echo "formatted, fully-qualified hostname."
    trailing_dot_msg
    ((input_errors++))
fi

if ! is_valid_forward_dns_name "$TARGET"; then
    >&2 echo "Target name '$TARGET' is not a correctly"
    >&2 echo "formatted, fully-qualified hostname."
    trailing_dot_msg
    ((input_errors++))
fi
...
process_curl_output $? "Create / update CNAME record operation failed:"